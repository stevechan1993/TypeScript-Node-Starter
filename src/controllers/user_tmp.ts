...
import {User, UserDocument ,AuthToken} from "../entity/User";
import {Request, Response, NextFunction} from "express";
import {IVerifyOptions} from "passport-local";
import {check, sanitize, validationResult} from "express-validator";
import "../config/passport_tmp";
...
export const postUpdateProfile = async (req: Request, res: Response, next: NextFunction) => {
  ...
  User.findOne({id: user.id}).then(user => {
    ...
    user.save().then(()=> {
      req.flash("success", {msg: "Profile informatiom has been updated."});
...
export const getOauthUnlink = (req: Request, res: Response, next: NextFunction) => {
  ...
  User.findOne({id: user.id}).then((user: any) => {
...
export const getReset = async (req: Request, res: Response, next: NextFunction) => {
  ...
  await getConnection()
    ...
    .where("user.passwordResetToken = :passwordResetToken", { passwordResetToken: req.params.token })
    .andWhere("user.passwordResetExpires > :passwordResetExpires", { passwordResetExpires: Date.now()})
    .execute().then(user => {
      if (!user) {
  ...
  async.waterfall([
    ...
    function sendResetPasswordEmail(user: UserDocument, done: Function) {
      ...
      const mailOptions = {
        ...
        text: `Hello,\n\nThis is a confirmation that the pasword for your accout ${user.email} has just been changed.\n`
      };
  ...
  async.waterfall([
    ...
    function sendForgotPasswordEmail(token: AuthToken, user: UserDocument, done: Function) {
      ...
      const mailOptions = {
        ...
        Please click on the following link, or paste this into your brower to complete the process:\n\n
